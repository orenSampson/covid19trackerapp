...
const { serverError, userCreated, userNotFound, wrongPassword, signinSuccessful } = require("../constants/responses");
...
exports.signup = async (req, res, next) => {
	...
	try {
		...
		return res.status(userCreated.status).json({ message: userCreated.message, userId: result._id });
	} catch (err) {
		return res.status(serverError.status).json({ message: serverError.message });
	}
};
...
exports.signin = async (req, res, next) => {
	...
	if (!user) {
		return res.status(userNotFound.status).json({ message: userNotFound.message });
	}
	...
	if (!isEqual) {
		return res.status(wrongPassword.status).json({ message: wrongPassword.message });
	}
